        ...
        if acc_1 > acc_2:
            ...
            if idx_drift == 0:
                ...
                for i, y_pred2 in enumerate(model2.staged_predict(x2)):
                    test_deviance2_former[i] = jaccard_score(y2, y_pred_former, average='micro')
                    
                
                # output latter loss 
                for i, y_pred2 in enumerate(model2.staged_predict(x_test)):
                    test_deviance2_latter[i] = jaccard_score(y2, y_pred_latter, average='micro')
                ...
                for i, y_pred2 in enumerate(model2.staged_predict(x_test[idx_drift:, :])):
                    test_deviance2_former[i] = jaccard_score(np.hstack((y2, y_test[idx_drift:])), y_pred_former, average='micro')
                    
                
                # output latter loss 
                for i, y_pred2 in enumerate(model2.staged_predict(np.vstack((x2, x_test[idx_drift:, :])))):
                    test_deviance2_latter[i] = jaccard_score(y_test[idx_drift:], y_pred_latter, average='micro')
...
if __name__ == '__main__':
    ...
    # datasets = ['powersupply', 'Poker-Hand', 'INSECTSa']
    # datasets = ['INSECTSg', 'INSECTSi', 'covtype']
    datasets = ['covtype']
    ...
    for i in range (len(datasets)):
        ...
        for j in range(5):
            ...
            ACC, F1 = DM_method(data, ini_train_size = 500, win_size = 500, seeds = j, name = datasets[i])
            time_end = time.time()
