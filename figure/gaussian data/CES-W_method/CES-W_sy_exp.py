...
def CES_method(data, ini_train_size, win_size, seeds, name):
    ...
    for train_index, test_index in tqdm(kf.split(stream), total = kf.get_n_splits(), desc = "#batch"):
        ...
        print('high warning', high_warning)
...
if __name__ == '__main__':
    ...
    for i in range (len(datasets)):
        ...
        for j in range(1):
            ...
            data = pd.read_csv('generated synthetic data/sud_data.txt', header = None)
            # data = pd.read_csv('generated synthetic data/inc_data.txt', header = None)
            
            print(datasets[i], 0)
            time_start = time.time()
            ACC, F1 = CES_method(data, ini_train_size = 4000, win_size = 4000, seeds = 0, name = datasets[i])
            time_end = time.time()
