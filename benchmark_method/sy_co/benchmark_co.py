...
def AEC_run (dataset_name, batch, seeds):
    ...
    model = AdditiveExpertEnsembleClassifier(n_estimators=10)
    while n_samples < max_samples and stream.has_more_samples():
        X, y = stream.next_sample(batch)
        y_pred = model.predict(X)
        pred = np.hstack((pred,y_pred))
...
def SRP_run (dataset_name, batch, seeds):
    ...
    result[:, 0] = pred[batch:]
    result[:, 1] = Y[batch:]
    np.savetxt(dataset_name +str(seeds) + '_'+'SRP.out', result, delimiter=',')
    
    return acc, f1


# for RTG, AGRa datasets
...
def RSL_run (dataset_name, batch, seeds):
    ...
    x_train, y_train = stream1.next_sample(100)
    ...
    result[:, 0] = pred[batch:]
    result[:, 1] = Y[batch:]
    np.savetxt(dataset_name +str(seeds) + '_'+'RSL.out', result, delimiter=',')
...
def OUO_run (dataset_name, batch, seeds):
    ...
    result[:, 0] = pred[batch:]
    result[:, 1] = Y[batch:]
    np.savetxt(dataset_name +str(seeds) + '_'+'OUO.out', result, delimiter=',')
...
if __name__ == '__main__':
    ...
    datasets = ['AGRa'] 
    # datasets = ['SEAa', 'RTG', 'RBF', 'RBFr', 'AGRa', 'HYP', 'LEDa', 'LEDg']
    batch = [100]
    ...
    for name in range(len(datasets)):
        ...
        for i in range (0,15):
           ...
           # print('time cost:', Time, 's')
            ...
            time_start = time.time()
            ACC, F1 = SRP1_run(dataset_name, batch_size, i) 
            time_end = time.time()
            Time = time_end - time_start
            print('time cost:', Time, 's')
